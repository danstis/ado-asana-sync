...
- `ADO_TIMEZONE`: IANA timezone name (e.g. `Pacific/Auckland`) that the ADO due-date instant is rendered as a calendar date in (default: `UTC`). `Microsoft.VSTS.Scheduling.DueDate` is a **DateTime** field, not a plain date — it always comes back as a UTC instant (trailing `Z`), and truncating that instant to its UTC calendar date lands on the wrong day when ADO stored the picked date as *local* midnight converted to UTC. **Only set this if you actually observe due dates arriving in Asana one day early**, which can happen when your ADO organisation/profile timezone is east of UTC (e.g. NZ at UTC+12/+13). Leaving it unset preserves today's exact behaviour. Setting a timezone *west* of UTC is never needed and can move already-correct dates backwards by a day, so don't. An invalid name logs a warning and falls back to `UTC`. Only newly created Asana tasks pick up a changed value — due dates are not re-sent on update. Date-only ADO values are passed through unchanged.
